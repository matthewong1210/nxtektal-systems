...
|---|---|---|---|
| Demand surge begins (2.5×, config window) | 14:00 | 479 | ~26.6 s |
| Autonomous dispatch decision (`assign_collection(R1,Z4)`) | 14:09 | 488 | ~27.1 s |
...
**0:27–0:32 — THE SURGE (14:00–15:30). Pause around sim time 15:00.**
An unforecast walk-in surge multiplies demand 2.5× — ~20 to ~50 balls/min,
peaking at 52.8/min at 15:07. The forecast the policy sees deliberately
excludes surges: it must detect the shortfall through the sensed inventory
pipeline and react. Watch the dispatch decisions staff collection; both
robots cycle collect→handoff on the map (a docking failure at 15:26 is
retried and absorbed). Resume.
...
to close. The washer loop never starves.
...
- **Final summary** tab: **maintained zero simulated stockout** — 0.0
  stockout minutes, 20,751/20,751 demanded balls served, 100% service
...
- Say: *"simulation results", "placeholder-provenance parameters",
  "reproducible from a seed", "forecast-based autonomous dispatch policy",
  "maintained zero simulated stockout", "the decision problem is real",
  "the environment is built to train and evaluate learned policies."*
- Never say or imply: real customer data, real facility throughput,
  validated hardware performance, field-measured costs, or that this
  policy is a learned/trained model.
