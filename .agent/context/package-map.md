...
| `apps/site-agent-console/` | Static Manager Console for the local fixture-backed Site Agent service; consumes only the versioned local Manager API | Typecheck, lint, Vitest, Next.js static-export build, HTTP smoke, production audit |
...
`nxt_workflow_enablement`, `nxt_course_world_model`, and
`nxt_site_agent` in the wheel.
...
| Site Agent service lifecycle, Manager API contract, projections, service-state files, or Manager Console | `site_agent_v0.md`, service contracts/guards, [deployment.md](deployment.md) | Entire `tests/site_agent`, agent-runtime and pilot-ops suites, architecture/safety suite, full suite, plus the console app's typecheck/lint/test/build/smoke |
