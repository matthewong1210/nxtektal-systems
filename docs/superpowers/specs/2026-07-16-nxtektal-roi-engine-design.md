...
5. **Candidate gating**: the schema rejects `input_status: "candidate"` values from formal calculation (allowed statuses: confirmed, estimated_allowed); rejected/superseded values are refused. Preliminary estimates count estimated_allowed and report X confirmed / Y estimated.
